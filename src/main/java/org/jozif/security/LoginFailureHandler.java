    ...
    public void onAuthenticationFailure(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, AuthenticationException authE) throws IOException, ServletException {
        try {
            ...
            if (null != loginFailureId && !"".equals(loginFailureId) && Integer.parseInt(loginFailureId) == 0) {
                httpServletResponse.sendRedirect(Global.SIGNIN_FAILURE_USERNOTFOUND_CONTROLLER + loginFailureId);
                return;
            }
            //密码不匹配
            if (security_code == Global.SIGNIN_FAILURE_WRONGPASSWORD_CODE) {
                httpServletResponse.sendRedirect(Global.SIGNIN_FAILURE_WRONGPASSWORD_CONTROLLER + loginFailureId);
                return;
            }
            //已停用
            if (security_code == Global.SIGNIN_FAILURE_USERSTOP_CODE) {//
                httpServletResponse.sendRedirect(Global.SIGNIN_FAILURE_USERSTOP_CONTROLLER + loginFailureId);
                return;
            }
            //已逻辑删除
            if (security_code == Global.SIGNIN_FAILURE_USERDELETE_CODE) {
                httpServletResponse.sendRedirect(Global.SIGNIN_FAILURE_USERDELETE_CONTROLLER + loginFailureId);
                return;
            }
            if (security_code == Global.SIGNIN_FAILURE_USERNOTFOUND_CODE){
                httpServletResponse.sendRedirect(Global.SIGNIN_FAILURE_USERNOTFOUND_CONTROLLER + 0);
                return;
            }
            //未知错误
            log.debug("未知错误authE: " + authE.getMessage());
            httpServletResponse.sendRedirect(Global.SIGNIN_FAILURE_UNKNOWN_CONTROLLER);
